...
class FilaEsperaService:
    ...
    async def entrar(
        ...
        """Adiciona paciente à fila de espera com status AGUARDANDO."""
        entrada = FilaEspera(
        ...
        self.db.add(entrada)
        await self.db.flush()
        log.info("fila_espera_entrada", paciente_id=paciente_id, id=entrada.id)
        return entrada

    async def sair(self, fila_id: int, paciente_id: int) -> None:
        """Remove paciente da fila (status → CANCELADO)."""
        ...
        if entrada:
            ...
            await self.db.flush()
            log.info("fila_espera_saida", fila_id=fila_id, paciente_id=paciente_id)
